...
int main(int argc, char **argv)
{
    ...
    // Transformation betweeen reference frame of camera and reference frame of EE.
    ...
    while (true)
    {
        if (robot_state_available && aruco_pose_available)
            // we subscribe to joint state topic
        {
            // Update robot
            robot.update(jnt_pos, jnt_vel);
            ...
            //position and orientation of aruco marker inside camera frame
        ...
        if(index == 0) {
            ...
            aruco_pos_n.normalize(); //to get s vector
            Eigen::Vector3d r_o = skew(Eigen::Vector3d(0,0,1))*aruco_pos_n; //cross product

            double aruco_angle = std::acos(Eigen::Vector3d(0,0,1).dot(aruco_pos_n)); //z dot s , and arccos
            ...
            off_frame.p = cam_T_object.p - KDL::Vector(0,0,0.4);
            off_frame.M = cam_T_object.M*KDL::Rotation::RotX(-3.14);

            KDL::Frame des_frame=robot.getEEFrame()*off_frame;

            Eigen::Matrix<double, 3, 1> e_o = computeOrientationError(toEigen(des_frame.M), toEigen(robot.getEEFrame().M));
            Eigen::Matrix<double, 3, 1> e_o_w = computeOrientationError(toEigen(Fi.M), toEigen(robot.getEEFrame().M));
            Eigen::Matrix<double, 3, 1> e_p = computeLinearError(toEigen(des_frame.p), toEigen(robot.getEEFrame().p));
            Eigen::Matrix<double, 6, 1> x_tilde;

            // e_o_w used to transform a 2D task in a 3D task, to don't rotate. In this way, we try to stay near the initial orientation.
            ...
            matr = (-1/P_c_o.norm())*(Eigen::Matrix<double,3,3>::Identity() - s*s.transpose());
            ...
            Eigen::MatrixXd LJ_pseudoinv  = LJ.completeOrthogonalDecomposition().pseudoInverse();
            Eigen::MatrixXd N  = Eigen::Matrix<double,7,7>::Identity() - LJ_pseudoinv*LJ;
            ...
            int k=5;
             ...
             // resolved velocity control low
            dqd.data = k*LJ_pseudoinv*s_d + N*(qdi - toEigen(jnt_pos));
